...
public class Fraction extends ExpressionModifier implements Expression {
    ...
    public Expression add(Expression expression) {
        if (expression instanceof Fraction f) {
            ...
            return oneCheck(new Fraction(tmp1.numerator.add(tmp2.numerator), tmp1.denominator));
        }
        ...
        return oneCheck(new Fraction(this.numerator.add(fraction.numerator).mul(new Number(1)), this.denominator));
    }
    ...
    public Expression sub(Expression expression) {
        if (expression instanceof Fraction f) {
            Fraction tmp1 = new Fraction(f.numerator.mul(this.denominator), f.denominator.mul(this.denominator));
            Fraction tmp2 = new Fraction(this.numerator.mul(f.denominator), this.denominator.mul(f.denominator));
            return oneCheck(new Fraction(tmp1.numerator.sub(tmp2.numerator), tmp1.denominator));
        }
        Fraction fraction = new Fraction(expression, new Number(1));
        fraction.numerator = fraction.numerator.mul(this.denominator);
        fraction.denominator = fraction.denominator.mul(this.denominator);
        return oneCheck(new Fraction(this.numerator.sub(fraction.numerator).mul(new Number(1)), this.denominator));
    }

    @Override
    public Expression mul(Expression expression) {
        if (expression instanceof Fraction f) {
            return oneCheck(new Fraction(this.numerator.mul(f.numerator), this.denominator.mul(f.denominator)));
        }
        return oneCheck(new Fraction(this.numerator.mul(expression), this.denominator));
    }
    ...
    private Expression oneCheck(Expression expression) {
        if (expression instanceof Fraction fraction) {
            if (fraction.numerator.equals(fraction.denominator)) {
                return new Number(1);
            }
        }
        return expression;
